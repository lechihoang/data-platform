...
def process(
    ...
    logger.info(
        f"Starting Data Quality Check for GOLD layer - Type: {dataset_type.upper()}"
    )
    ...
    validate_table_with_ge(
        gx_context,
        df_monthly,
        f"monthly_summary_{dataset_type}",
        [
            gx.expectations.ExpectColumnValuesToNotBeNull(column="total_trips"),
            gx.expectations.ExpectColumnValuesToBeBetween(
                column="total_trips", min_value=1, max_value=99999999
            ),
    ...
    if dataset_type != "fhv":
        revenue_expectations.append(
            gx.expectations.ExpectColumnValuesToNotBeNull(column="total_revenue")
        )

    validate_table_with_ge(
        gx_context, df_revenue, f"revenue_by_zone_{dataset_type}", revenue_expectations
    )
