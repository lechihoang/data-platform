...
def build_facts(spark, logger, dataset_type, target_year, target_month):
    df = (
        spark.table("nessie.silver.trips")
        .filter(
    ...
    fact_daily_trips = df.groupBy("Year", "Month", "trip_date", "trip_type").agg(
    ...
    fact_monthly_summary = df.groupBy("Year", "Month", "trip_type").agg(
        F.count("*").alias("total_trips"),
        F.sum("trip_distance").alias("total_distance"),
        F.sum("total_amount").alias("total_revenue"),
        F.avg("trip_duration_seconds").alias("avg_trip_duration_seconds"),
    ...
    fact_revenue_by_zone = (
        ...
        .withColumn(
            "tip_percentage",
            F.when(F.col("total_fare") == 0, 0).otherwise(
                (F.col("total_tip") / F.col("total_fare")) * 100
            ),
        )
    )
    ...
    write_partitioned_table(
        fact_daily_trips, "nessie.gold.fact_daily_trips", ["Year", "Month", "trip_type"]
    )
    write_partitioned_table(
        fact_monthly_summary,
        "nessie.gold.fact_monthly_summary",
        ["Year", "Month", "trip_type"],
    )
    write_partitioned_table(
        fact_revenue_by_zone,
        "nessie.gold.fact_revenue_by_zone",
        ["Year", "Month", "trip_type"],
    )
    write_partitioned_table(
        fact_payment_summary,
        "nessie.gold.fact_payment_summary",
        ["Year", "Month", "trip_type"],
    )
...
def process(
    ...
    logger.info(
        f"STARTING SILVER TO GOLD FOR {dataset_type.upper()} (Branch: {branch_name})"
    )
