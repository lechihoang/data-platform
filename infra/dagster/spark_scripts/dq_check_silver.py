...
def process(
    ...
    logger.info(
        f"Starting Data Quality Check for SILVER layer - Type: {dataset_type.upper()}"
    )
    ...
    if dataset_type == "zone":
        ...
        logger.info(
            f"Checking table nessie.silver.trips for {dataset_type} ({target_year}-{target_month:02d})"
        )
        ...
        validate_table_with_ge(
            ...
            [
                ...
                gx.expectations.ExpectColumnValuesToBeBetween(
                    column="trip_duration_seconds", min_value=0, max_value=86400
                ),
            ],
        )
        ...
        if dataset_type in ["yellow", "green", "hvfhv"]:
            validate_table_with_ge(
                gx_context,
                df_trips,
                f"trips_{dataset_type}_fares",
                [
                    gx.expectations.ExpectColumnValuesToNotBeNull(
                        column="total_amount"
                    ),
                    gx.expectations.ExpectColumnValuesToBeBetween(
                        column="total_amount", min_value=0, max_value=99999
                    ),
                    gx.expectations.ExpectColumnValuesToBeBetween(
                        column="passenger_count", min_value=1, max_value=9, mostly=0.9
                    ),
                    gx.expectations.ExpectColumnValuesToBeBetween(
                        column="trip_distance", min_value=0, max_value=150, mostly=0.9
                    ),
                ],
            )
    ...
    return {
        ...
        "TARGET_PERIOD": (
            f"{target_year}-{target_month:02d}" if dataset_type != "zone" else "N/A"
        ),
