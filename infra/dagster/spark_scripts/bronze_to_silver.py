...
class AbstractProcessor(ABC):
    ...
    def __init__(
        self, spark: SparkSession, logger: logging.Logger, params: Dict[str, Any]
    ):
        self.spark = spark
        self.logger = logger
        self.params = params
...
class TripProcessor(AbstractProcessor):
    ...
    def transform(self, df: DataFrame) -> DataFrame:
        ...
        df_enriched = (
            df_clean.withColumn("Year", F.year(F.col("pickup_datetime")))
...
class YellowTripProcessor(TripProcessor):
    def __init__(
        ...
        super().__init__(
            spark, logger, params, trip_type="yellow", file_prefix="yellow_tripdata"
        )
...
class GreenTripProcessor(TripProcessor):
    def __init__(
        ...
        super().__init__(
            spark, logger, params, trip_type="green", file_prefix="green_tripdata"
        )
...
class FHVTripProcessor(TripProcessor):
    def __init__(
        ...
        super().__init__(
            spark, logger, params, trip_type="fhv", file_prefix="fhv_tripdata"
        )
...
class HVFHVTripProcessor(TripProcessor):
    def __init__(
        ...
        super().__init__(
            spark, logger, params, trip_type="hvfhv", file_prefix="fhvhv_tripdata"
        )
...
def initialize_nessie_branch(
    spark: SparkSession, branch_name: str, logger: logging.Logger
):
...
def process(
    ...
    logger.info(
        f"Successfully processed {processed_rows} rows for {dataset_type} to branch {branch_name}"
    )
