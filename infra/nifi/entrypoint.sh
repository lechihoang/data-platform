...
until [ "$(curl -s -k -o /dev/null -w "%{http_code}" https://$(hostname):8443/nifi-api/access/config)" != "000" ]; do
    sleep 5
done
...
TOKEN=$(curl -s -k -X POST https://$(hostname):8443/nifi-api/access/token -H "Content-Type: application/x-www-form-urlencoded" --data "username=${SINGLE_USER_CREDENTIALS_USERNAME:-admin}&password=${SINGLE_USER_CREDENTIALS_PASSWORD}")

# Lấy Root Process Group ID
ROOT_PG=$(curl -s -k -X GET https://$(hostname):8443/nifi-api/flow/process-groups/root -H "Authorization: Bearer $TOKEN" | grep -o '"id":"[^"]*' | head -1 | awk -F'"' '{print $4}')

# Lấy danh sách Process Group con
PG_LIST=$(curl -s -k -X GET https://$(hostname):8443/nifi-api/process-groups/$ROOT_PG/process-groups -H "Authorization: Bearer $TOKEN")
...
for flow_file in /opt/nifi/nifi-current/flows/*.json; do
    ...
    if echo "$PG_LIST" | grep -q "\"name\":\"$flow_name\""; then
        ...
        curl -s -k -X POST -H "Authorization: Bearer $TOKEN" https://$(hostname):8443/nifi-api/process-groups/$ROOT_PG/process-groups/upload -F "id=$ROOT_PG" -F "clientId=$(cat /proc/sys/kernel/random/uuid)" -F "groupName=$flow_name" -F "positionX=0" -F "positionY=0" -F "file=@$flow_file" > /dev/null
